...
/** 새 대화 시작용 퀵 카테고리 */
const QUICK_STARTS = [
  { emoji: '🛒', label: '쇼핑 주문', prompt: '온라인에서 물건을 주문하고 싶어요' },
  { emoji: '🏦', label: '은행/송금', prompt: '모바일뱅킹으로 송금하는 방법을 알려주세요' },
  { emoji: '🚄', label: 'KTX 예매', prompt: 'KTX 기차표를 예매하고 싶어요' },
  { emoji: '📱', label: '스마트폰 설정', prompt: '스마트폰 글자 크기를 키우고 싶어요' },
  { emoji: '🏥', label: '병원 예약', prompt: '병원 진료 예약을 하고 싶어요' },
  { emoji: '🔒', label: '피싱 확인', prompt: '이상한 전화를 받았는데 사기인지 확인해주세요' },
  { emoji: '🍔', label: '음식 주문', prompt: '배달 음식을 주문하고 싶어요' },
  { emoji: '💡', label: '기타 질문', prompt: '' },
];
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <header className="bg-white border-b border-gray-200 px-4 py-4 shrink-0">
        <div className="max-w-2xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-lg font-bold text-gray-900">AI 집사 대화</h1>
          </div>
        </div>
      </header>
      ...
      <main className="flex-1 overflow-y-auto max-w-2xl mx-auto w-full px-4 py-6">
        {/* 새 대화 시작 */}
        <div className="mb-8">
          <h2 className="text-base font-bold text-gray-900 mb-3">새 대화 시작하기</h2>
          <div className="grid grid-cols-2 gap-2.5">
            {QUICK_STARTS.map((item) => (
              <button
                key={item.label}
                onClick={() => handleNewChat(item.prompt)}
                className="bg-white border border-gray-200 rounded-xl p-3.5 text-left hover:border-primary-300 hover:bg-primary-50 transition-all active:scale-[0.98]"
              >
        ...
        {sessions.length > 0 && (
          <div>
            <h2 className="text-base font-bold text-gray-900 mb-3">
              이전 대화 <span className="text-gray-400 font-normal text-sm">({sessions.length})</span>
            </h2>
